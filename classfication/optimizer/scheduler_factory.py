...
# _log_message = logging.getlog_message(__name__)
...
class CosineLRScheduler(Scheduler):
    ...
    def __init__(
        ...
        if t_initial == 1 and cycle_mul == 1 and cycle_decay == 1:
            ...
            print("Cosine annealing scheduler will have no effect on the learning rate since t_initial = t_mul = eta_mul = 1.")
...
def scheduler_kwargs(cfg, decreasing_metric: Optional[bool] = None):
    ...
    kwargs = dict(
        sched=cfg.sched,
        num_epochs=getattr(cfg, 'train_epochs', 100),
