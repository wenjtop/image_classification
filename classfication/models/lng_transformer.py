...
from optimizer.weight_init import trunc_normal_
import time
...
def window_partition(x, window_size):
    B, H, W, C = x.shape
    h_window_num = H // window_size
    w_window_num = W // window_size

    x = x.view(B, h_window_num, window_size, w_window_num, window_size, C)
    x = x.permute(0, 1, 3, 2, 4, 5).contiguous()
    x = x.view(B, h_window_num * w_window_num, window_size * window_size, C)  # B, hn*wn, w*w, C
    return x
...
def window_reverse(x, window_size, H, W):
    ...
    h_window_num = H // window_size
    w_window_num = W // window_size
    x = x.view(B, h_window_num, w_window_num, window_size, window_size, C)
    x = x.permute(0, 1, 3, 2, 4, 5).contiguous()
    x = x.view(B, h_window_num*window_size, w_window_num*window_size, C)  # B, H, W, C
    return x
...
class Attention(nn.Module):
    ...
    def generate_mask(self, w, p, shift):
        attn_mask = torch.zeros(w, w, p, p, p, p, dtype=torch.bool, device=self.relative_position_index.device)
        s = p - shift
        ...
        attn_mask = attn_mask.view(1, 1, w*w, p*p, p*p)
        return attn_mask
    ...
    def forward(self, x, type=None):
        ...
        if type == 'S':
            attn_mask = self.generate_mask(int(math.sqrt(N)), int(math.sqrt(S)), shift=self.window_size//2)
...
class LNGAttentionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.LocalAttention(x)
        x = self.window_reverse(x, self.window_size, H, W)        # input：B, N, S, C  。return：B, C, H, W

        # Neighbor
        x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        x = self.window_partition(x, self.window_size)            # input：B, H, W ,C。 return：B, N, S, C
        x = self.NeighborAttention(x, type='S')
        x = self.window_reverse(x, self.window_size, H, W)        # input：B, N, S, C  。return：B, C, H, W
        x = torch.roll(x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))

        # Global
        x = self.window_partition(x, self.window_stage_size)      # input：B, H, W ,C。 return：B, N, S, C
        x = x.permute(0, 2, 1, 3)
        x = self.GlobalAttention(x)
        x = x.permute(0, 2, 1, 3)
        x = self.window_reverse(x, self.window_stage_size, H, W)  # input：B, N, S, C  。return：B, C, H, W
...
class LLAttentionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.window_partition(x, self.window_size)      # input：B, H, W ,C。 return：B, N, S, C
        x = self.LocalAttention1(x)
        x = self.LocalAttention2(x)
        x = self.window_reverse(x, self.window_size, H, W)  # input：B, N, S, C  。return：B, C, H, W
...
def LNG_T(num_classes=100, config=[1, 1, 1, 1], dim=45, **kwargs):
    return LNGTransformer(in_chans=3, dims=[dim, dim*2, dim*4, dim*8], patch_size=4, window_size=7, stages=config, num_heads=[3, 6, 12, 24], num_classes=num_classes)

def LNG_S(num_classes=100, config=[1, 1, 2, 1], dim=128, **kwargs):
    return LNGTransformer(in_chans=3, dims=[dim, dim*2, dim*4, dim*8], patch_size=4, window_size=7, stages=config, num_heads=[4, 8, 16, 32], num_classes=num_classes)
