...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        # x: [B, L, C]
        x, H, W = self.patch_embed(x)   # [8, 3136, 96]  56, 56
        x = self.pos_drop(x)
...
def Swin_T(num_classes: int = 100, **kwargs):
    # trained ImageNet-1K
...
def Swin_S(num_classes: int = 100, **kwargs):
    # trained ImageNet-1K
...
def Swin_B(num_classes: int = 100, **kwargs):
    # trained ImageNet-1K
...
if __name__ == '__main__':
    ...
    # print(test_model)
    dummy_input = torch.rand(1, 3, 224, 300).cuda()
